...
async def _(almhebevent):
    if almhebevent.fwd_from:
        return
    almhebcc = Faker()
    almhebname = almhebcc.name()
    almhebadre = almhebcc.address()
    almhebcard = almhebcc.credit_card_full()

    await edit_or_reply(almhebevent,
                        f"𓆰 𝙎𝙊𝙐𝙍𝘾𝞝 almheb - 𝙑𝙄𝙎𝘼_𝘾𝘼𝙍𝘿  💳𓆪\n\n\n__**👤 الاسـم :- **__\n ٴ──┄──┄──┄──┄──\n`{almhebname}`\n\n__**🏡 العنـوان :- **__\n ٴ──┄──┄──┄──┄──\n`{almhebadre}`\n\n__**💸 الفيـزا :- **__\n ٴ──┄──┄──┄──┄──\n`{almhebcard}`\n\n◟𝙎𝙤𝙪𝙧𝙘𝙚 𝙕𝙚𝙙𝙏𝙝𝙤𝙣 ➧ @ZedThon◞ ")
