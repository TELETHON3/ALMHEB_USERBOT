...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
        ...
        if query.startswith("**ZedBot") and event.query.user_id == bot.uid:
            buttons = [
                (
                    ...
                    Button.url("Repo", "https://github.com/Guevaras1explanations/ALMHEB_USERBOT"),
                )
            ]
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            reply_pop_up_alert = "⌔∮ عليك الحصول على بوت almheb للحصول عليه اذهب الى @X888E. "
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            reply_pop_up_alert = "⌔∮ عليك الحصول على بوت almheb للحصول عليه اذهب الى @X8888E. "
            await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    async def on_plug_in_callback_query_handler(event):
        ...
        if os.path.exists("./userbot/secrets.txt"):
            ...
            reply_pop_up_alert = "⌔∮ هذه الرساله لم تعـد موجوده "
        await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            reply_pop_up_alert += "استخدم .الغاء اضافه {} لحذف هذه الاضافه ".format(
                plugin_name
            )
            try:
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
