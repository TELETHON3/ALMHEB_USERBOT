...
@bot.on(admin_cmd(pattern="ت$", outgoing=True))
@bot.on(sudo_cmd(pattern="ت$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
CMD_HELP.update(
    {
        "افتارات تطقيم بنات": "**اسم الاضافـه : **`افتارات تطقيم بنات`\
    \n\n**╮•❐ الامـر ⦂ **`.ت` بالـرد علـى رقـم من 1 الى 29 \
    \n**الشـرح •• **تحميل افتـارات تطقيـم بنـات تمبلـر ممطـروقـه"
    }
)